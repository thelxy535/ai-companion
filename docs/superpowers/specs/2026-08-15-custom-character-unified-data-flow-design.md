...
`previewCapsule` 只用于告别页展示，不写入数据库。真正删除时由 `deletePermanently` 在一个 Room 事务中生成最终胶囊并删除运行数据；不把“先写胶囊”和“稍后删除”拆成两个可中断的持久化步骤。

删除必须在数据库事务中执行：

1. 校验角色属于当前用户。
2. 根据用户选择在同一事务中生成胶囊（未选择保留胶囊时跳过）。
3. 删除消息标签关联。
4. 删除该角色消息。
5. 删除角色记忆、向量记忆、情绪/关系状态、交互时间和事件。
6. 删除头像偏好、草稿和缓存。
7. 删除角色实体。
8. 删除内存人格注册。
9. 提交事务并返回收据。

如果胶囊保存失败或任一删除步骤失败，事务回滚，原角色继续可用。不可恢复删除必须保留确认对话框。
