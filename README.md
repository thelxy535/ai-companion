...
> 一个更像“在和人相处”的原生 Android AI 伴侣。

弦曜 SYLORA 不把陪伴拆成机械的任务列表。每个角色都有自己的性格底色、说话节奏、生活线、记忆取舍和主动联系边界；用户可以使用内置角色，也可以创建完全属于自己的角色。
...
| 能力 | 体验重点 |
| --- | --- |
| 独立人格 | 不同角色拥有不同的敏感度、表达欲、慢热程度、分享欲和关系边界 |
| 连续场景 | 默认延续上一条的地点、穿着、姿态和正在做的事，不会因为过了几分钟就突然换场景 |
| 真实记忆 | 对话、长期记忆、关系叙事、情绪余波和生活线共同影响后续交流 |
| 自然主动 | 角色可以在合适的时候主动联系，但会考虑打扰成本，不为了完成指标硬发消息 |
| 自定义角色 | 角色卡、Character Book、示例对话、深层人格、语音和行为规则都可编辑 |
| 数据自主 | 聊天和记忆默认保存在本地，模型服务由用户自行配置 |
...
当前项目处于持续开发阶段，重点方向是让不同角色保持更稳定的个体差异，让长期记忆召回更自然，并提升后台主动消息和系统通知的可靠性。
